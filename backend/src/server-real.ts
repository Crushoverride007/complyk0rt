...
import { frameworkStructures } from './frameworks'
...
const generalLimiter = rateLimit({
  ...
  skip: (req: any) => {
    ...
    if (p.startsWith('/assessments/') && (p.endsWith('/answers') || p.endsWith('/structure'))) return true
    return false
  },
})
if (!devMode) app.use(generalLimiter)
...
function isAdmin(req: any): boolean {
  ...
  const roles = (req.user?.rolesByOrg || []) as any[]
  if (email.toLowerCase() === 'admin@acme.example.com') return true
  return roles.some(r => String(r.role || '').toLowerCase() === 'admin')
}
function getCollaboratorRole(assessmentId: string, userId: string) {
  try {
    const fs = require('fs')
    ...
    const found = list.find((c: any) => c.userId === userId)
    return found?.role || null
  } catch { return null }
}
function canReadAssessment(req: any, assessmentId: string): boolean {
  if (isAdmin(req)) return true;
  const ur = (req.user?.rolesByOrg || []) as any[]
  ...
  if (rbac.includes(role)) return true
  const collab = getCollaboratorRole(assessmentId, req.user?.userId)
  return !!collab // viewer or editor
}
function canWriteAssessment(req: any, assessmentId: string): boolean {
  if (isAdmin(req)) return true;
  const ur = (req.user?.rolesByOrg || []) as any[]
  const orgId = req.user?.orgId || ur[0]?.orgId || null
  const role = ur.find(r => r.orgId === orgId)?.role || req.user?.role || 'viewer'
...
// --- RBAC: Roles and auth helpers ---
type Role = 'admin'|'manager'|'viewer'|'guest'|'qa'|'assessor'|'consultant'|'team_leader'
const RolePolicy: Record<Role, string[]> = {
...
function requireAuth(req: any, res: any, next: any) {
  try {
    ...
    req.user = payload
    const headerOrg = (req.headers['x-org-id'] as string) || (req.headers['x-organization'] as string)
    if (headerOrg) req.user.orgId = headerOrg
    next()
  } catch {
...
function authorizeRoles(roles: Role[]){
  return (req: any, res: any, next: any) => {
    ...
    const orgId = req.user?.orgId || ur[0]?.orgId || null
    const role = ur.find(r => r.orgId === orgId)?.role || req.user?.role || 'viewer'
    if ((roles as string[]).includes(role) || (req.user?.email === 'admin@acme.example.com' && (!Array.isArray(req.user?.rolesByOrg) || !req.user.rolesByOrg.length))) return next()
...
// Who am I / roles
app.get('/api/me/roles', requireAuth as any, async (req, res) => {
  try {
...
app.post('/api/auth/login', loginLimiter as any, async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { userId: user.id, email: user.email, name: user.name, rolesByOrg: memberships },
...
// Get all users
app.get('/api/users', requireAuth as any, authorizeRoles(['admin','team_leader','manager']) as any, async (req, res) => {
  try {
    const ur = (req.user?.rolesByOrg || []) as any[]
    ...
    if (!orgId) return res.status(400).json({ success:false, message:'orgId not resolved' })
...
app.post('/api/users/invite', requireAuth as any, authorizeRoles(['admin','team_leader','manager']) as any, async (req, res) => {
  try {
    ...
    const rolesByOrg = (req.user?.rolesByOrg || []) as any[]
    let orgId = bodyOrgId || req.user?.orgId || (rolesByOrg[0]?.orgId)
    if (!orgId) {
      const anyOrg = await prisma.organization.findFirst({ select: { id: true } })
      orgId = anyOrg?.id || null
    }
    if (!orgId) return res.status(400).json({ success:false, message:'orgId not resolved (provide x-org-id header or body.orgId)' })
...
app.get('/api/orgs/:orgId/members', requireAuth as any, async (req, res) => {
  try {
...
app.put('/api/orgs/:orgId/members/:userId', requireAuth as any, authorizeRoles(['admin','team_leader','manager']) as any, async (req, res) => {
  try {
    const { orgId: orgParam, userId } = req.params
    const { role } = req.body || {}
    const rolesAllowed = ['viewer','guest','admin','qa','assessor','consultant','team_leader','manager']
    if (!rolesAllowed.includes(role)) return res.status(400).json({ success:false, message:'Invalid role' })
    let orgId = orgParam
    if (!/^[-0-9a-f]{36}$/i.test(orgId)) {
      const org = await prisma.organization.findFirst({ where: { OR: [{ id: orgId }, { slug: orgId }] }, select: { id:true } })
      orgId = org?.id || orgId
...
app.put('/assessments/:id', requireAuth as any, async (req, res) => {
  try {
    const { id } = req.params
    ...
    const dataPath = path.join(__dirname, '../data.json')
    const data = JSON.parse(fs.readFileSync(dataPath, 'utf8'))
    const aIdx = (data.assessments || []).findIndex((a:any) => a.id === id)
    if (aIdx === -1) return res.status(404).json({ success:false, message:'Assessment not found' })
    const a = data.assessments[aIdx]
    if (typeof title === 'string') a.title = title
    if (typeof col === 'string') a.col = col
    if (typeof dueIn === 'string') a.dueIn = dueIn
    if (typeof framework === 'string') a.framework = framework
    if (typeof description === 'string') a.description = description
    if (typeof assignedTo === 'string') a.assignedTo = assignedTo
    if (typeof template === 'string') a.template = template
...
app.delete('/assessments/:id', requireAuth as any, async (req, res) => {
  try {
    const { id } = req.params
    if (!canWriteAssessment(req, id)) return res.status(403).json({ success:false, message:'Forbidden' })
    ...
    if (hard) {
      ...
      if (data.tasksByAssessment) delete data.tasksByAssessment[id]
      if (data.messagesByAssessment) delete data.messagesByAssessment[id]
      if (data.attachmentsByAssessment) delete data.attachmentsByAssessment[id]
      if (data.answersByAssessment) delete data.answersByAssessment[id]
      if (data.sectionAttachmentsByAssessment) delete data.sectionAttachmentsByAssessment[id]
      fs.writeFileSync(dataPath, JSON.stringify(data, null, 2))
...
app.put('/assessments/:id/unarchive', requireAuth as any, async (req, res) => {
  try {
    const { id } = req.params
    if (!canWriteAssessment(req, id)) return res.status(403).json({ success:false, message:'Forbidden' })
...
app.get('/assessments/:id/tasks', async (req, res) => {
  try {
    const { id } = req.params
    if (!canReadAssessment(req, id)) return res.status(403).json({ success:false, message:'Forbidden' })
...
const maybeAuthMessages: any = (process.env.DEV_ALLOW_UNAUTH === 'true' ? ((_:any, __:any, next:any)=>next()) : (requireAuth as any));
...
app.post('/assessments/:id/messages', requireAuth as any, async (req, res) => {
  try {
    const { id } = req.params
    if (!canWriteAssessment(req, id)) return res.status(403).json({ success:false, message:'Forbidden' })

    const { user = 'System', text = '', parentId = null, sections = [], attachments = [], mentions = [] } = req.body || {}
    if (!text || typeof text !== 'string') return res.status(400).json({ success:false, message:'text required' })
...
// Delete a message
app.delete('/assessments/:id/messages/:msgId', requireAuth as any, async (req, res) => {
  try {
    ...
    if (!data.messagesByAssessment) data.messagesByAssessment = {}
...
app.delete('/assessments/:id/attachments/:attId', requireAuth as any, async (req, res) => {
  try {
    ...
    const idx = list.findIndex((a:any) => a.id === attId)
    if (idx === -1) return res.status(404).json({ success:false, message:'Attachment not found' })
    const removed = list.splice(idx, 1)[0]
    if (!data.attachmentsByAssessment) data.attachmentsByAssessment = {}
    ...
    if (data.sectionAttachmentsByAssessment && data.sectionAttachmentsByAssessment[id]) {
      ...
      for (const key of Object.keys(mapping)) {
        mapping[key] = (mapping[key] || []).filter((x:string) => x !== attId)
      }
    }
    fs.writeFileSync(dataPath, JSON.stringify(data, null, 2))
    return res.json({ success:true, removed })
...
app.get('/assessments/:id/structure', async (req, res) => {
  try {
    ...
    if (!structure) {
      const assessment = (data.assessments || []).find((a: any) => a.id === id)
      const fwName = assessment?.framework || 'PCI DSS 4.0'
      const fw = (frameworkStructures as any)[fwName]
...
app.get('/assessments/:id/answers', requireAuth as any, async (req, res) => {
  try {
    const { id } = req.params
    if (!canReadAssessment(req, id)) return res.status(403).json({ success:false, message:'Forbidden' })
...
app.put('/assessments/:id/answers', requireAuth as any, answersLimiter as any, async (req, res) => {
  try {
    const { id } = req.params
...
app.get('/assessments/:id/collaborators', requireAuth as any, async (req, res) => {
  try {
    const { id } = req.params
    const fs = require('fs'); const path = require('path');
    const dataPath = path.join(__dirname, '../data.json');
...
app.post('/assessments/:id/collaborators', requireAuth as any, async (req, res) => {
  try {
    const { id } = req.params; const { userId, role } = req.body || {};
    if (!userId || !role || !['viewer','editor'].includes(role)) return res.status(400).json({ success:false, message:'userId and role (viewer|editor) required' });
    ...
    const list = mapping[id] = Array.isArray(mapping[id]) ? mapping[id] : [];
    const idx = list.findIndex((c:any)=>c.userId===userId);
    if (idx === -1) list.push({ userId, role }); else list[idx].role = role;
    fs.writeFileSync(dataPath, JSON.stringify(data, null, 2));
...
app.delete('/assessments/:id/collaborators/:userId', requireAuth as any, async (req, res) => {
  try {
    ...
    const filtered = list.filter((c:any)=>c.userId!==userId);
    mapping[id] = filtered;
    fs.writeFileSync(dataPath, JSON.stringify(data, null, 2));
...
app.get('/assessments/:id/permissions', requireAuth as any, (req, res) => {
  const { id } = req.params
...
app.get('/assessments/:id/sections/:subId/attachments', requireAuth as any, async (req, res) => {
  try {
    ...
    const resolved = all.filter((a:any) => set.has(a.id))
...
app.put('/assessments/:id/sections/:subId/attachments', requireAuth as any, async (req, res) => {
  try {
    const { id, subId } = req.params
    if (!canWriteAssessment(req, id)) return res.status(403).json({ success:false, message:'Forbidden' })
    ...
    const allIds = new Set(((data.attachmentsByAssessment||{})[id] || []).map((a:any)=>a.id))
    ;(Array.isArray(add)?add:[]).forEach((attId:any)=>{ if (allIds.has(attId)) list.add(attId) })
    ;(Array.isArray(remove)?remove:[]).forEach((attId:any)=>{ list.delete(attId) })
    current[subId] = Array.from(list)
    fs.writeFileSync(dataPath, JSON.stringify(data, null, 2))
    const resolved = ((data.attachmentsByAssessment||{})[id] || []).filter((a:any)=>list.has(a.id))
...
app.delete('/assessments/:id/sections/:subId/attachments/:attId', requireAuth as any, async (req, res) => {
  try {
...
function readData() {
  const fs = require('fs'); const path = require('path');
  const dataPath = path.join(__dirname, '../data.json');
  let data = {} as any; try { data = JSON.parse(fs.readFileSync(dataPath, 'utf8')) } catch {}
...
function normalizeRole(role: string) {
  ...
  if (allowed.includes(r)) return r === 'guest' ? 'viewer' : r;
  // legacy mappings
  if (r === 'contributor' || r === 'member' || r === 'editor') return 'assessor';
  return 'viewer';
}
...
app.get('/api/users', (req, res) => {
  ...
  const { data } = readData();
  const users = (data.users || []).map((u:any)=>({ id:u.id, email:u.email, name:u.name, role:u.role||'viewer', active: u.active!==false }));
  return res.json({ success:true, data: users });
});

app.post('/api/users', (req, res, next) => {
  if (process.env.DEV_ALLOW_UNAUTH === 'true') return next();
  return (requireAuth as any)(req, res, next);
}, (req, res, next) => {
  if (process.env.DEV_ALLOW_UNAUTH === 'true') return next();
  const ok = (req.user && (req.user.email === 'admin@acme.example.com' || ((req.user.rolesByOrg||[]).some((r:any)=>['admin','manager'].includes(String(r.role).toLowerCase())))));
  ...
  try {
    ...
    // upsert membership
    const existing = await prisma.membership.findFirst({ where: { userId: user.id, orgId } })
    if (!existing) await prisma.membership.create({ data: { userId: user.id, orgId, role: desiredRole } })
    else if (existing.role !== desiredRole) await prisma.membership.update({ where: { id: existing.id }, data: { role: desiredRole } })
...
app.put('/api/users/:id', (req, res, next) => {
  ...
  try {
    ...
    if (role !== undefined) {
      ...
      if (!existing) await prisma.membership.create({ data: { userId, orgId, role: desiredRole } })
      else if (existing.role != desiredRole) await prisma.membership.update({ where: { id: existing.id }, data: { role: desiredRole } })
    }
    // active toggle (admin only)
    if (active !== undefined) {
      const isAdminRole = (req.user && (req.user.email === 'admin@acme.example.com' || ((req.user.rolesByOrg||[]).some((r:any)=>String(r.role).toLowerCase()==='admin'))))
      if (!(process.env.DEV_ALLOW_UNAUTH === 'true') && !isAdminRole) return res.status(403).json({ success:false, message:'Forbidden (admin required for activation change)' })
...
app.delete('/api/users/:id', (req, res, next) => {
  ...
  try {
    // admin only
    const isAdminRole = (req.user && (req.user.email === 'admin@acme.example.com' || ((req.user.rolesByOrg||[]).some((r:any)=>String(r.role).toLowerCase()==='admin'))))
    if (!(process.env.DEV_ALLOW_UNAUTH === 'true') && !isAdminRole) return res.status(403).json({ success:false, message:'Forbidden' })
